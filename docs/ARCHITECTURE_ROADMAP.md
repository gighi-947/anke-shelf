...
| 项 | 现状 |
| --- | --- |
| 功能基线 | `5fdc46c`（android: SettingsScreen 大屏拆分；2026-08-14 核对） |
| 分支 | `main`，本地领先 `origin/main` 1 提交（`5fdc46c` 待推送） |
...
> parts 不入 APK；`SettingsScreen.kt` 已拆分（1369 → 575 行，三个 Panel 文件）。
> DownloadScreen / NativeReaderScreen 与 Windows 前端拆分待办。
