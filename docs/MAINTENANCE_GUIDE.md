...
| --- | --- | --- |
| Windows Python | 328 项（3.14：仅环境性 main_guard 失败（tasklist 沙箱不可用，与代码无关）；bundled 3.12 全过） | `python -m unittest discover tests` |
...
| Android JVM | 217 项（216 过 / 1 跳）+ DisciplineTest 11 项 | `cd android && gradlew.bat testDebugUnitTest` |
