...
| JS 契约 | textpos 15 例、api-contract 53 方法、launch 诊断、bridge v1、parts 6/37377B、reader-session OK | `node contracts/tests/*.test.js` |
...
- 基线 `main`（HEAD 以 `git log` 为准）；`win/gululu-reader-interaction` 已并入主干；
  最近主线为骨碌碌适配（v1.3.0 / v1.4.0）、五批接手风险修复与 P5 批次（P5-A 快赢批）；
