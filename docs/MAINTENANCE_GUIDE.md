...
| 端 | 当前版本 | 技术栈 | 版本源位置 |
| --- | --- | --- | --- |
| Windows | **v1.5.0** | Python + Web SPA + pywebview 壳 | 多文件散落（`app/__init__.py`、bridge MOCK、版本测试、README 等，升级按清单全量替换） |
...
- 基线 `main`（HEAD 以 `git log` 为准）；`win/gululu-reader-interaction` 已并入主干；
  最近主线为骨碌碌适配（v1.3.0 / v1.5.0）、五批接手风险修复、P5 批次（P5-A/B/D/E1/E2-Android）、NGA 主题自适应与深浅色下载选项移除与
  多轮架构收敛（ApiError / TaskManager / reader-lite 状态机 / MOCKS 移除）；
  文档漂移治理已强化（AGENTS §5 高漂移清单 + `scripts/check-doc-drift.ps1`）。
- 版本线：Windows v1.5.0、Android android-v1.1.0，均已发布。
