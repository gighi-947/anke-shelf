...
| --- | --- | --- | --- | --- |
| Windows 桌面端 | **v1.6.1** | Python + Web（前后端分离，pywebview 壳） | `vX.Y.Z` | `AnkeShelf-vX.Y.Z.zip` |
| Android 手机端 | **v1.3.1** | Kotlin + Jetpack Compose（正文用安卓专用 WebView 渲染内核） | `android-vX.Y.Z` | `AnkeShelf-vX.Y.Z-android.apk` |
...
从 [GitHub Releases](https://github.com/gighi-947/anke-shelf/releases) 下载
`AnkeShelf-v1.3.1-android.apk`（Android 8.0+ / API 26+）安装。
...
- **版本线分离**：Windows 用 `vX.Y.Z`（当前 v1.6.1）；Android 用
  `android-vX.Y.Z`（当前 v1.3.1），唯一版本定义在
