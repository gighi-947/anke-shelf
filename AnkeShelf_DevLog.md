...
- 需求：取消自定义 uid 多值过滤，仅保留「只看楼主」开关（两个模式）；
  暂缓多 uid，待后续「非主楼回复评论化」开发时再研究。
- 关键结论（回退依据）：NGA 的 `lou` 楼号在带 `authorid` 过滤（只看楼主）
  请求时被**重新编号为过滤序号**，与默认模式的全局楼号是两套坐标体系；
  此前“过滤/全部混用”的改造方向错误，已整体回退到改动前基线再重做。
- 新设计（两个模式 + 模式锁定）：
  - 下载：`author_only` 开关，开启后自动获取楼主 uid（NGA `tauthorid`，
    无过滤拉第 1 页 `probe.user_id` / `first.authorId`）作 `authorid` 过滤；
    关闭则无过滤拉全部楼层。
  - 更新：开关锁定为书模式（文件夹名 `tid(uid)`，uid>0 即只看楼主书），
    后端 `_update_core` 固定 `author_id`、移除“更新时切换过滤”；
    Android `NgaDownloadService` 更新分支固定 `authorId = defaults.authorId`。
  - 模式切换：去下载页重新下载（两模式是不同坐标体系的两本书）。
- 数据契约不变：`author_id`（文件夹名/meta/settings）仍作模式标识，
  老书（手填 uid 下载的）天然兼容开关。
- 验证：Windows Python 342 项全过；Android JVM 238 项（237 过 / 1 跳）；
  双端编译通过；无回归（测试计数回到基线）。
...
- 追加修复（真机五轮反馈）：离屏 WebView 的 `file:///android_asset` CSS
...
- 追加修复（真机九轮）：「分享本批」只分享本次导出的文件列表，
  不再扫描目录下所有图片，重进页面自动清空；楼层筛选支持「N」或
  「N楼」精确匹配楼号，避免输入 1楼 命中 7001楼/6991楼。
- 追加修复（真机十轮，日志定位）：导出「按当前阅读设定」调用
...
- 现象（用户反馈）：① 首次导出失败后「开始导出」按钮永久禁用；②
  「分享」提示回导出页选楼层；③ 导出主题缺少「按当前阅读设定」，
  自定义主题无法生效；④ 导出页 UI 不符合规范。
- 修复：
  - 前端 `floor_export.js`：开始导出用 try/catch 恢复按钮；分享按钮
    直接携带当前楼层号，不再依赖导出页已选楼层；主题新增
    「按当前阅读设定」并读取阅读器实际 CSS 变量（含自定义主题/跟随
    系统）传入后端；导出页改用 `download-body` + `settings-section`
...
- 修复：
  - Windows：`web/js/gululu-immersive.js` 的音乐入口与「音乐与氛围」面板
    不再只对骨碌碌书源开放——非骨碌碌书一旦有音乐 cue 播放，顶栏即显示
    音乐入口；面板只保留音量 / 进度 / 播放暂停 / 停止音乐，自动音乐、
    氛围背景、动态视效仅骨碌碌书可见。停止后自动收起入口与面板。
    `web/index.html` 给三个骨碌碌专属开关行加 id 供切换。
  - Android：非骨碌碌书新增右下角音乐悬浮入口（MusicNote FAB），点击弹出
    「正在播放：标题 + 停止音乐」菜单，复用骨碌碌悬浮菜单同款样式与令牌；
    悬浮栏收起时菜单同步关闭。
  - 冒烟脚本 `formal_ui_smoke.js` 的 NGA 隔离断言同步更新：音乐 cue
    应可播放且显示音乐入口，秘密等骨碌碌能力仍保持隔离。
- 验证：Windows Python 338 项、Android JVM 235 项（234 过 / 1 跳）、
  JS 契约/守卫全绿；`node --check` 通过；未推真机（待构建安装）。
...
- 应要求重写 `nga-post-template.bbcode`：太长不看、核心功能、阅读体验、
  当前版本、安装使用等小节重新组织。
- 当前版本小节明确写出 v1.6.2 / android-v1.3.2 的修复内容（NGA 楼层卡片
  灰边 / 卡底丢失），并回溯 v1.6.1 / v1.3.1 的性能与包体优化。
- 核心功能新增“沉浸阅读”一条：骰点遮罩、迷雾、秘密、线索、段落评论、
  音乐 / 氛围背景 / 视效。
